...
const createShop = async (req, res) => {
  ...
  try {
    ...
    await sendMail({
      ...
      message: `
      Welcome to GraniMart! \n
      Hi ${shopData.name}\n
      Thank you for registering with GraniMart. To activate your Seller account, please use the following One-Time Password (OTP):${otp}\n
    ...
    return res.status(500).json({
      success: false,
      msg: "An error occurred while creating the Shop.",
    });
  }
};
...
const activeShop = async (req, res) => {
  ...
  try {
    ...
    if (shop) {
      ...
      await sendMail({
        email: shopData.email,
        subject: "Welcome to GraniMart",
        text: `
      Welcome to GraniMart!\n
      Hi ${shopData.name},\n
      We're thrilled to welcome you to GraniMart! Your seller account has been successfully activated, and you're now part of our growing community.\n      
      Seller Name: ${shopData.name}\n
      Email: ${shopData.email}\n      
      If you have any questions or need assistance, feel free to reach out to our support team at <a href="mailto:support@granimart.com">support@granimart.com</a>.\n
      Thank you for choosing GraniMart.
      Best regards,\n
      The GraniMart Team\n`,
      });
