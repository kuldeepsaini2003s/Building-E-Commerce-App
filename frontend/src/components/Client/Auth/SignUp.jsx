      ...
      try {
        ...
        if (status === 200) {
          handleResponse({
            ...
            showToast: true,
            onSuccess: () => {
              sessionStorage.setItem("activationToken", data?.activation_Token);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-8 sm:px-6 lg:px-8">
      <div className="sm:mx-auto place-items-center sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-4 text-center text-3xl font-extrabold text-gray-900">
          {verificationSection ? "OTP Verification" : "Register as a new user"}
        </h2>
      </div>
      <div className="mt-5 sm:mx-auto sm:w-full sm:max-w-md">
        <div
          className="bg-white py-6 px-4 shadow-md
        rounded-lg sm:px-10"
        >
          {!verificationSection ? (
            <form action={submitAction} className="space-y-4">
              ...
              <div>
                <button
                  ...
                  } w-full h-[40px] flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700`}
                >
                  Submit
                </button>
              </div>
